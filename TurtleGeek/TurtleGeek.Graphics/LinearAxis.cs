...
namespace TurtleGeek.Graphics
{
	...
	public class LinearAxis : Axis, System.ICloneable
	{
		...
		public override object Clone()
		{
			...
			// ensure that this isn't being called on a derived type.
			if (this.GetType() != a.GetType())
			{
				throw new ApplicationException( "Clone not defined in derived type of LinearAxis." );
			}
			this.DoClone( this, a );
			return a;
		}
			...
			)
		{
			...
			if (tickDist < 0.0) // some sanity checking. TODO: remove this.
				throw new ApplicationException( "Tick dist cannot be negative." );
			...
			if ( !double.IsNaN(largeTickStep_) )
			{
				if ( largeTickStep_ <= 0.0f )
				{
					throw new ApplicationException( 
						"Cannot have negative or zero tick step; reverse WorldMin WorldMax instead."
					);
				}
		...
		private int DetermineNumberSmallTicks( double bigTickDist )
		{
			...
			{
				throw new ApplicationException( "Mantissa.Length must equal SmallTickCounts.Length." );
			}
