	...
	public class HistogramPlot : BaseSequencePlot, IPlot, ISequencePlot
	{
		...
		public float BaseWidth
		{
			...
			set
			{
				...
				{
					throw new ApplicationException( "Base width must be between 0.0 and 1.0." );
				}
			}
		}
		...
		*/
		...
		public void StackedTo(HistogramPlot hp)
		{
			...
			SequenceAdapter hpData = 
				...
				if ( hp != null )
				{
					...
					{
						throw new ApplicationException("HistogramPlot data can only be stacked with the same number of datapoints.");
					}
					for ( int i=0; i < data.Count; ++i )
					{
						if ( data[i].X != hpData[i].X )
						{
                            throw new ApplicationException("HistogramPlot data can only be stacked with the same X coordinates.");
						}
						if ( hpData[i].Y < 0.0f)
						{
                            throw new ApplicationException("HistogramPlot data can only be stacked with positive Y coordinates.");
						}
					}
				}
				stackedTo_ = hp;
		...
		public void DrawInLegend( System.Drawing.Graphics g, Rectangle startEnd )
		{
