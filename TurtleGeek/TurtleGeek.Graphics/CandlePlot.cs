...
namespace TurtleGeek.Graphics
{
	...
	public class CandlePlot : BasePlot, IPlot
	{
        ...
        public class CandleDataAdapter
		{
			...
			public PointOLHC this[int i]
			{
				get
				{
                    ...
                    {
                        throw new ApplicationException("Unexpected data source.");
                    }
				...
				get
				{
                    ...
                    if (openData_ is Array)
                    {
                        ...
                        if (size != ((Array)closeData_).Length)
                            throw new ApplicationException("openData and closeData arrays are not of same length.");
                        if (size != ((Array)lowData_).Length)
                            throw new ApplicationException("openData and lowData arrays are not of same length.");
                        if (size != ((Array)highData_).Length)
                            throw new ApplicationException("openData and highData arrays are not of same length.");
                        return size;
                    }

                    throw new ApplicationException( "Data not in correct format." );
				}
			}
        ...
        public virtual void DrawInLegend( System.Drawing.Graphics g, Rectangle startEnd)
		{
			Pen	p =	new	Pen(this.color_);
		...
		public int StickWidth
		{
			...
			set
			{
				if (value < 1)
				{
					throw new ApplicationException( "Stick width must be greater than 0." );
				}
				stickWidth_ = value;
			}
		}
