...
Public Class HatchlingsDoaEventInformationDlg
...
#Region " Windows Form Designer generated code "
    ...
    Friend WithEvents colTagHatchlingsDoaEventTagIDDataGridViewTextBoxColumn As System.Windows.Forms.DataGridViewTextBoxColumn
    Friend WithEvents colTagHatchlingsDoaEventIDDataGridViewTextBoxColumn As System.Windows.Forms.DataGridViewTextBoxColumn
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(HatchlingsDoaEventInformationDlg))
        ...
        '
        'HatchlingsDoaEventInformationDlg
        '
        ...
        Me.Name = "HatchlingsDoaEventInformationDlg"
        Me.ShowInTaskbar = False
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Hatchlings DOA Event"
...
#End Region
    ...
    Private moHatchlingsDoaEvent As HatchlingsDoaEvent

    Public Sub New(ByVal oHatchlingsDoaEvent As HatchlingsDoaEvent)
        ...
        moHatchlingsDoaEvent = oHatchlingsDoaEvent

    End Sub

    Private Sub frmHatchlingsDoaEventInformationDlg_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        Try
            '*** ASSERTION ***
            If moHatchlingsDoaEvent Is Nothing Then
                Throw New ApplicationException("HatchlingsDoaEvent object must be set before opening this screen.")
            End If
            '*** ASSERTION ***
            ...
            '----------------------------------------------------------------
            moHatchlingsDoaEvent.BeginEdit()

            Me.cboSpeciesList.SelectedIndex = cboSpeciesList.FindString(moHatchlingsDoaEvent.SpeciesCode)

            Me.dtpEventDate.DateValue = moHatchlingsDoaEvent.EventDate

            If (moHatchlingsDoaEvent.EventCount = 0) Then
                Me.txtEventCount.Text = String.Empty
            Else
                Me.txtEventCount.Text = Convert.ToString(moHatchlingsDoaEvent.EventCount)
            End If

            Me.cboCountyList.SelectedIndex = cboCountyList.FindString(moHatchlingsDoaEvent.DoaFromCounty)
    ...
    Private Sub frmHatchlingsDoaEventInformationDlg_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
    ...
    Private Function DoCancelLogic() As Boolean
        ...
        Try
            ...
            '------------------------------------------------------------ 
            If moHatchlingsDoaEvent.IsDirty Then
            ...
            '------------------------------------------------------------ 
            moHatchlingsDoaEvent.CancelEdit()
    ...
    Private Function DoSaveLogic() As Boolean
        ...
        '------------------------------------------------------------ 
        Dim oTempHatchlingsDoaEvent As HatchlingsDoaEvent
        ...
        Try
            ...
            If moHatchlingsDoaEvent.IsDirty Then
                ...
                '------------------------------------------------------------ 
                If moHatchlingsDoaEvent.IsValid Then

                    '----------------------------------------------------------------
                    '-- commit edits in memory
                    '----------------------------------------------------------------
                    moHatchlingsDoaEvent.ApplyEdit()

                    '----------------------------------------------------------------
                    '-- clone and save the object (it is the CSLA way)
                    '----------------------------------------------------------------
                    oTempHatchlingsDoaEvent = moHatchlingsDoaEvent.Clone
                    moHatchlingsDoaEvent = oTempHatchlingsDoaEvent.Save
    ...
    Private Sub DisplayBrokenRules(Optional ByVal bShowMessageBox As Boolean = False)
        ...
        Try
            ...
            If (bShowMessageBox) Then
                MessageBox.Show(" *** The following data validation issue(s) must be resolved before this package can be saved: *** " & ControlChars.CrLf & ControlChars.CrLf & moHatchlingsDoaEvent.GetBrokenRulesString, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            End If

            For i = 0 To moHatchlingsDoaEvent.BrokenRulesCollection.Count() - 1
                Select Case moHatchlingsDoaEvent.BrokenRulesCollection(i).Property
                    Case "EventCount"
                        erpErrorProvider.SetError(Me.txtEventCount, moHatchlingsDoaEvent.BrokenRulesCollection(i).Description)
                End Select
            Next
    ...
    Private Sub CopyFormValuesToBusinessObject()
        ...
        If cboSpeciesList.SelectedIndex > -1 Then
            moHatchlingsDoaEvent.SpeciesCode = CType(Me.cboSpeciesList.SelectedItem, KeyValueInfo).Key
        End If

        moHatchlingsDoaEvent.EventDate = Me.dtpEventDate.DateValue
        moHatchlingsDoaEvent.EventCount = Convert.ToInt32(Val(Me.txtEventCount.Text.Trim))
        moHatchlingsDoaEvent.DoaFromCounty = Me.cboCountyList.Text.Trim

    End Sub

    Private Sub btnHelp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnHelp.Click
        Help.ShowHelp(Me, gsHelpFileName, "HatchlingsDoaEventScreen.htm")
    End Sub
